...
3. For each **format script** (document/file generation -- PDF, HTML, XLSX, CSV, etc.):
   ...
   python3 -m forge.scripts.src.scaffold add-script \
     --root output/{folder_name} \
     --name gen_csv.py \
     --script path/to/gen_csv.py \
     --test path/to/test_gen_csv.py \
     --deps dep1,dep2 \
     --install
   ```
...
5. After ALL scripts are generated, run the full test suite. ALL tests must pass.
...
- Confirmation that all scripts were generated and placed
