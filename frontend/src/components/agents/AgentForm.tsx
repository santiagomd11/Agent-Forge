...
export function AgentForm({ agent }: AgentFormProps) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6 max-w-2xl">
      ...
      {isEditing && (
        <>
          <SchemaEditor label="Input Schema" fields={inputSchema} onChange={setInputSchema} isInput={true} />
          <SchemaEditor label="Output Schema" fields={outputSchema} onChange={setOutputSchema} isInput={false} />
        </>
      )}
